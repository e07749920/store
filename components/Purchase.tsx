  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8">
        <div className="flex justify-between items-center">
            ...
            <button onClick={() => setIsModalOpen(true)} className="px-6 py-3 bg-blue-600 text-slate-900 dark:text-white font-bold rounded-xl flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {purchaseOrders.map(po => (
                <div key={po.id} className="glass-panel p-6 rounded-3xl border border-slate-200 dark:border-white/5 relative">
                     <div className="flex justify-between mb-4">
                        <h3 className="font-bold text-white">{po.itemName}</h3>
                        <span className="text-xs font-bold text-slate-600 dark:text-slate-400 px-2 py-1 bg-slate-200 dark:bg-white/10 rounded">{po.status}</span>
                     </div>
                     <div className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                        Qty: {po.quantity} | Supplier: {po.supplier} | Cost: {formatCurrency(po.totalCost)}
                     </div>
                     {po.status === 'ORDERED' && (
                         <button onClick={() => handleReceivePO(po)} disabled={isSaving} className="w-full py-2 bg-emerald-600 text-slate-900 dark:text-white font-bold rounded-lg">
        ...
        {isModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative glass-panel p-8 rounded-3xl w-full max-w-lg bg-[#050505]">
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Create Purchase Order</h3>
                    <form onSubmit={handleCreatePO} className="space-y-4">
                        <select required value={poItemId} onChange={e => setPoItemId(e.target.value)} className="w-full bg-black/50 border border-slate-300 dark:border-white/10 rounded-xl px-4 py-3 text-white">
                            <option value="">Select Item...</option>
                            {items.map(i => <option key={i.id} value={i.id}>{i.name} ({i.materialNo})</option>)}
                        </select>
                        <input type="number" required value={poQty} onChange={e => setPoQty(parseInt(e.target.value))} className="w-full bg-black/50 border border-slate-300 dark:border-white/10 rounded-xl px-4 py-3 text-white" placeholder="Quantity" />
                        <input type="text" required value={poSupplier} onChange={e => setPoSupplier(e.target.value)} className="w-full bg-black/50 border border-slate-300 dark:border-white/10 rounded-xl px-4 py-3 text-white" placeholder="Supplier" />
                        <button type="submit" disabled={isSaving} className="w-full py-3 bg-blue-600 text-slate-900 dark:text-white font-bold rounded-xl">Create Order</button>
                    </form>
                </div>
            </div>
        )}
