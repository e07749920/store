      ...
      return (
        <div className="h-[80vh] flex items-center justify-center">
            <div className="glass-panel p-12 text-center">
                ...
                <p className="text-slate-600 dark:text-slate-400">Stock Opname is restricted to authorized personnel.</p>
            </div>
        </div>
      );
  }
      ...
      {view === 'LIST' && (
          <>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    <h2 className="text-3xl font-bold text-slate-900 dark:text-white font-[Space_Grotesk]">Stock Opname Sessions</h2>
                    <p className="text-slate-600 dark:text-slate-400 text-sm font-mono mt-2">Audit Management & Reconciliation</p>
                </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Create New Card */}
                <div className={`glass-panel p-6 rounded-3xl border ${hasOpenSession ? 'border-slate-700 opacity-80' : 'border-slate-300 dark:border-white/10'}`}>
                    ...
                    <form onSubmit={handleCreateSession} className="space-y-4">
                        <div>
                            <label className="text-xs font-bold text-slate-600 dark:text-slate-500 uppercase">Session Title</label>
                            <input 
                                type="text" required 
                                value={newTitle} onChange={e => setNewTitle(e.target.value)}
                                placeholder="e.g., Q1 2024 Audit - Spare Parts"
                                disabled={hasOpenSession}
                                className="w-full bg-slate-100 dark:bg-black/40 border border-slate-300 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white outline-none focus:border-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                        </div>
                        <div>
                            <label className="text-xs font-bold text-slate-600 dark:text-slate-500 uppercase">Notes</label>
                            <textarea 
                                value={newNotes} onChange={e => setNewNotes(e.target.value)}
                                disabled={hasOpenSession}
                                className="w-full bg-slate-100 dark:bg-black/40 border border-slate-300 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white outline-none focus:border-emerald-500 h-24 resize-none disabled:opacity-50 disabled:cursor-not-allowed"
                            ></textarea>
                        </div>
                        <button disabled={isCreating || hasOpenSession} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-600 dark:text-slate-400 text-slate-900 dark:text-white font-bold rounded-xl transition-all flex justify-center items-center gap-2">
                ...
                {/* History List */}
                <div className="lg:col-span-2 glass-panel p-6 rounded-3xl border border-slate-300 dark:border-white/10 min-h-[400px]">
                    ...
                    <div className="space-y-3 overflow-y-auto max-h-[500px] custom-scrollbar pr-2">
                        {sessions.map(session => (
                            <div key={session.id} className="p-4 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:bg-white/10 transition-colors flex justify-between items-center group">
                                <div>
                                    <h4 className="font-bold text-slate-900 dark:text-white text-lg">{session.title}</h4>
                                    <div className="flex gap-4 text-xs text-slate-600 dark:text-slate-400 mt-1 font-mono">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button onClick={() => handleOpenSession(session)} className="p-2 bg-indigo-600/20 text-indigo-400 rounded-xl hover:bg-indigo-600 hover:text-slate-900 dark:text-white transition-all">
                                        <ArrowRight className="w-5 h-5" />
                                    </button>
                                </div>
                            </div>
                        ))}
                        {sessions.length === 0 && !isLoading && <p className="text-slate-600 dark:text-slate-500 text-center py-10">No sessions found.</p>}
                    </div>
                </div>
            </div>
          </>
      ...
      {view === 'SESSION' && currentSession && (
          <div className="animate-in fade-in slide-in-from-right-8 duration-500">
              ...
              <div className="flex flex-col xl:flex-row items-start xl:items-center gap-4 mb-6">
                  <button onClick={() => setView('LIST')} className="p-2 bg-slate-100 dark:bg-white/5 rounded-xl text-slate-600 dark:text-slate-400 hover:text-white">
                  ...
                  <div className="flex flex-wrap gap-3 w-full xl:w-auto">
                      <button 
                        ...
                        disabled={isLoading}
                        className="flex-1 xl:flex-none px-5 py-3 bg-emerald-700/80 hover:bg-emerald-600 text-slate-900 dark:text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg"
                      >
                      ...
                      {currentSession.status === 'OPEN' && (
                          <>
                              <button 
                                ...
                                className="flex-1 xl:flex-none px-5 py-3 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10 text-slate-900 dark:text-white font-bold rounded-xl flex items-center justify-center gap-2"
                              >
                                  <Save className="w-5 h-5" /> Save Draft
                              </button>
                              <button 
                                onClick={handleFinalize}
                                className="flex-1 xl:flex-none px-5 py-3 bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-bold rounded-xl shadow-lg flex items-center justify-center gap-2"
                              >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex flex-col">
                      <span className="text-slate-600 dark:text-slate-400 text-xs font-bold uppercase mb-1">Progress</span>
                      ...
                      <div className="w-full bg-slate-100 dark:bg-white/5 h-1 mt-2 rounded-full overflow-hidden">
                          <div className="bg-indigo-500 h-full transition-all duration-500" style={{ width: `${progressPercentage}%` }}></div>
                      </div>
                  </div>
                  <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex flex-col">
                      <span className="text-slate-600 dark:text-slate-400 text-xs font-bold uppercase mb-1">Accuracy</span>
                      <div className="flex items-end gap-2">
                  ...
                  <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex flex-col">
                      <span className="text-slate-600 dark:text-slate-400 text-xs font-bold uppercase mb-1">Items Counted</span>
                      <div className="flex items-end gap-2">
                          <span className="text-2xl font-bold text-white">{stats.counted}</span>
                          <span className="text-sm text-slate-600 dark:text-slate-500 mb-1">/ {stats.total}</span>
                      </div>
                  </div>
                  <div className="glass-panel p-4 rounded-2xl border border-slate-200 dark:border-white/5 flex flex-col">
                      <span className="text-slate-600 dark:text-slate-400 text-xs font-bold uppercase mb-1">Variance Detected</span>
                      <div className="flex items-end gap-2">
              ...
              <div className="glass-panel rounded-3xl overflow-hidden border border-slate-300 dark:border-white/10">
                  {/* Toolbar & Filters */}
                  <div className="p-4 border-b border-slate-300 dark:border-white/10 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-100 dark:bg-black/20">
                      <div className="flex items-center gap-4 w-full md:w-auto">
                          <div className="relative flex-1 md:flex-none">
                              ...
                              <input 
                                ...
                                className="bg-slate-100 dark:bg-black/40 border border-slate-300 dark:border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500 w-full md:w-64" 
                              />
                          </div>
                          
                          <div className="flex bg-slate-100 dark:bg-black/40 rounded-lg p-1 border border-slate-300 dark:border-white/10">
                              <button 
                                onClick={() => setStatusFilter('ALL')}
                                className={`px-3 py-1 rounded text-xs font-bold transition-all ${statusFilter === 'ALL' ? 'bg-indigo-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-white'}`}
                              >
                                  All
                              </button>
                              <button 
                                onClick={() => setStatusFilter('UNCOUNTED')}
                                className={`px-3 py-1 rounded text-xs font-bold transition-all ${statusFilter === 'UNCOUNTED' ? 'bg-amber-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-white'}`}
                              >
                                  Uncounted
                              </button>
                              <button 
                                onClick={() => setStatusFilter('COUNTED')}
                                className={`px-3 py-1 rounded text-xs font-bold transition-all ${statusFilter === 'COUNTED' ? 'bg-emerald-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:text-white'}`}
                              >
                                  Counted
                              </button>
                          </div>
                      ...
                      <div className="flex items-center gap-4 text-sm text-slate-600 dark:text-slate-400 w-full md:w-auto justify-end">
                          <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1} className="hover:text-slate-900 dark:text-white disabled:opacity-30"><ChevronLeft className="w-5 h-5" /></button>
                          <span>Page {page} of {Math.ceil(totalItems / pageSize) || 1}</span>
                          <button onClick={() => setPage(p => p+1)} disabled={page >= Math.ceil(totalItems / pageSize)} className="hover:text-slate-900 dark:text-white disabled:opacity-30"><ChevronRight className="w-5 h-5" /></button>
                      </div>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-100 dark:bg-black/40 text-xs uppercase text-slate-600 dark:text-slate-400 font-bold">
                            <tr>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {sessionItems.map(item => {
                                ...
                                return (
                                    <tr key={item.id} className={`hover:bg-slate-100 dark:bg-white/5 ${item.isCounted ? 'bg-indigo-900/5' : ''}`}>
                                        <td className="px-6 py-4">
                                            <div className="font-bold text-white">{item.materialDesc}</div>
                                            <div className="text-xs text-slate-600 dark:text-slate-500 font-mono">{item.materialNo}</div>
                                        </td>
                                        <td className="px-6 py-4 font-mono text-slate-700 dark:text-slate-300">{item.sloc}</td>
                                        ...
                                        <td className="px-6 py-4 text-center bg-indigo-900/5">
                                            <input 
                                                ...
                                                className={`w-24 bg-black/50 border border-white/20 rounded-lg py-2 text-center text-slate-900 dark:text-white font-bold outline-none focus:border-indigo-500 focus:bg-black/80 ${item.isCounted ? 'border-indigo-500/50' : ''}`}
                                            />
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            {item.isCounted ? (
                                                ...
                                                <span className="text-slate-600 text-xs bg-slate-100 dark:bg-white/5 px-2 py-1 rounded border border-slate-200 dark:border-white/5">Pending</span>
                                            )}
                                        </td>
                                    </tr>
                                );
                            ...
                            {sessionItems.length === 0 && !isLoading && (
                                <tr><td colSpan={6} className="text-center py-8 text-slate-600 dark:text-slate-500">No items match your filters.</td></tr>
                            )}
                        </tbody>
                    </table>
                  </div>
                  {isLoading && <div className="p-4 text-center text-slate-600 dark:text-slate-400 flex justify-center gap-2"><Loader2 className="animate-spin" /> Loading Data...</div>}
              </div>
          </div>
      )}
    </div>
